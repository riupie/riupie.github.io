...
host master01.openshift.riupie.io { hardware ethernet 52:54:00:58:b0:a9; fixed-address 10.50.50.11; }
host master02.openshift.riupie.io { hardware ethernet 52:54:00:79:70:08; fixed-address 10.50.50.12; }
host master03.openshift.riupie.io { hardware ethernet 52:54:00:de:92:a2; fixed-address 10.50.50.13; }

host worker01.openshift.riupie.io { hardware ethernet 52:54:00:6f:4b:1f; fixed-address 10.50.50.21; }
host worker02.openshift.riupie.io { hardware ethernet 52:54:00:c7:f4:21; fixed-address 10.50.50.22; }

host bootstrap { hardware ethernet 52:54:00:7a:55:a1; fixed-address 10.50.50.10; }
...
frontend machine-config-server
    bind api.openshift.riupie.io:22623
...
```
 ...
 #### 3.1. Set NTP pool server on each gluster nodes
```bash
vim /etc/chrony.conf
```
```

pool 10.50.50.9 iburst

```
#### 3.2. Partition and format disk
```bash
#Partition /dev/vdb disk
parted /dev/vdb mklabel msdos
parted -a opt /dev/vdb mkpart primary xfs 0% 100%

#Format /dev/vdb1 partition
mkfs.xfs /dev/vdb1 -L gfsdisk

#Mount /dev/vdb1 partition
mkdir -p /export/gfsbrick
mount /dev/vdb1 /export/gfsbrick/

#Add /export/gfsbrick/ mount point to /etc/fstab. Make it persistent after reboot.
echo "/dev/vdb1 /export/gfsbrick/ xfs defaults 1 2" >> /etc/fstab
...
```
#### 3.4. Enable glusterd service
```bash
systemctl enable --now glusterd
systemctl status glusterd
```

#### 3.5. Mapping /etc/hosts on each LB nodes
```bash
vim /etc/hosts
```
```
...
10.50.50.31 ag-okd-lb1
10.50.50.32 ag-okd-lb2
```

#### 3.6. Configure GlusterFS Trusted Storage Pool. Execute on  `ag-okd-lb1` node.
```bash
gluster peer probe ag-okd-lb2
```

#### 3.7. Verify glusterfs cluster
```bash
gluster peer status
gluster pool list
```
...
```
#### 1.14. After bootstrapping complete, check installation progress.
```bash
...
#### 1.15. Logging in to the cluster
```bash
